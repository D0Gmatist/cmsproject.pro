...
final class AuthorizationVk {
	...
	private function getVkUserIdAndToken () {
		...
		if ( (int)$token['user_id'] > 0 AND isset( $token['access_token'] ) ) {
			if ( $this->searchDouble ( $token['user_id'] ) > 0 ) {
				...
				$this->msgBox->getResult ( false, $this->language['authorization_vk'][1], 'error' );
	...
	private function complete () {
		...
		if ( (int)$this->memberId['user_id'] > 0 ) {
			...
			$this->functions->setCookie( "user_vk", md5( md5( $obj['user_vk_token'] ) ), 365 );
