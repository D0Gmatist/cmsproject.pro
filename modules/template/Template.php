...
class Template implements TemplateInterface {
    ...
    public function loadTemplate( $tpl_name ) {
        ...
        if ( strpos( $this->template, '[not-smartphone]' ) !== false ) {
            $this->template = preg_replace_callback( "#\\[(not-smartphone)\\](.*?)\\[/not-smartphone\\]#is", array( &$this, 'checkDevice' ), $this->template );
        }
        if ( strpos( $this->template, '[tablet]' ) !== false) {
            $this->template = preg_replace_callback( "#\\[(tablet)\\](.*?)\\[/tablet\\]#is", array( &$this, 'checkDevice' ), $this->template );
        }
        if ( strpos( $this->template, '[not-tablet]' ) !== false ) {
            $this->template = preg_replace_callback( "#\\[(not-tablet)\\](.*?)\\[/not-tablet\\]#is", array( &$this, 'checkDevice' ), $this->template );
        }
        if ( strpos( $this->template, '[desktop]' ) !== false ) {
            $this->template = preg_replace_callback( "#\\[(desktop)\\](.*?)\\[/desktop\\]#is", array( &$this, 'checkDevice' ), $this->template );
        }
        if ( strpos( $this->template, '[not-desktop]' ) !== false) {
            $this->template = preg_replace_callback( "#\\[(not-desktop)\\](.*?)\\[/not-desktop\\]#is", array( &$this, 'checkDevice' ), $this->template );
        }
    ...
    public function loadFile( $matches = [] ) {
        ...
        if ( $this->include_mode == 'php' ) {
            ...
            $url['query'] = str_ireplace( array( 'file_path', 'file_name', '_GET', '_FILES', '_POST', '_REQUEST', '_SERVER', '_COOKIE', '_SESSION' ) , 'Filtered', $url['query'] );
    ...
    public function subLoadTemplate( $tpl_name ) {
        ...
        if ( strpos( $template, "[not-smartphone]" ) !== false ) {
            $template = preg_replace_callback( "#\\[(not-smartphone)\\](.*?)\\[/not-smartphone\\]#is", array( &$this, 'checkDevice' ), $template );
        }
        if ( strpos( $template, "[tablet]" ) !== false ) {
            $template = preg_replace_callback( "#\\[(tablet)\\](.*?)\\[/tablet\\]#is", array( &$this, 'checkDevice' ), $template );
        }
        if ( strpos( $template, "[not-tablet]" ) !== false ) {
            $template = preg_replace_callback( "#\\[(not-tablet)\\](.*?)\\[/not-tablet\\]#is", array( &$this, 'checkDevice' ), $template );
        }
        if ( strpos( $template, "[desktop]" ) !== false ) {
            $template = preg_replace_callback( "#\\[(desktop)\\](.*?)\\[/desktop\\]#is", array( &$this, 'checkDevice' ), $template );
        }
        if ( strpos( $template, "[not-desktop]" ) !== false ) {
            $template = preg_replace_callback( "#\\[(not-desktop)\\](.*?)\\[/not-desktop\\]#is", array( &$this, 'checkDevice' ), $template );
        }
        return $template;
    }
    ...
    public function checkModule( $matches ) {
        ...
        return preg_replace_callback( $regex, array( &$this, 'checkModule' ), $matches );
    }
